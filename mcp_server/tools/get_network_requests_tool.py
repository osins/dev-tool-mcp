"""
Get Network Requests Tool - 获取网络请求工具
"""
import json
...
def create_get_network_requests_tool() -> MCPTool:
    """创建 GetNetworkRequestsTool 实例"""
    tool = Tool(
    ...
    async def handler(arguments: dict, progress_callback: Callable[[str], Awaitable[None]]) -> list:
        try:
            ...
            if not isinstance(result, (dict, list, str)):
                raise ValueError("Result from get_network_requests is not in expected format")
            ...
            # 处理值错误
            error_msg = f"Value Error in get_network_requests tool: {str(e)}"
            return [TextContent(type="text", text=error_msg)]
        except TypeError as e:
            # 处理类型错误
            error_msg = f"Type Error in get_network_requests tool: {str(e)}"
            return [TextContent(type="text", text=error_msg)]
        except Exception as e:
            # 处理其他异常
            error_msg = f"Unexpected error in get_network_requests tool: {str(e)}"
            return [TextContent(type="text", text=error_msg)]
