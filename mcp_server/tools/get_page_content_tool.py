"""
Get Page Content Tool - 获取页面内容工具
"""
import json
...
def create_get_page_content_tool() -> MCPTool:
    """创建 GetPageContentTool 实例"""
    tool = Tool(
    ...
    async def handler(arguments: dict, progress_callback: Callable[[str], Awaitable[None]]) -> list:
        try:
            ...
            if not isinstance(result, (dict, list, str)):
                raise ValueError("Result from get_page_content is not in expected format")
            ...
            # 处理值错误
            error_msg = f"Value Error in get_page_content tool: {str(e)}"
            return [TextContent(type="text", text=error_msg)]
        except TypeError as e:
            # 处理类型错误
            error_msg = f"Type Error in get_page_content tool: {str(e)}"
            return [TextContent(type="text", text=error_msg)]
        except Exception as e:
            # 处理其他异常
            error_msg = f"Unexpected error in get_page_content tool: {str(e)}"
            return [TextContent(type="text", text=error_msg)]
